// WNBA calibration page — Phase 78. Mirrors MLB calibration exactly:
// predicted-vs-observed buckets by probability tier, stat type, card
// type. Bayesian-smoothed hit rates so thin samples don't fake-claim
// accuracy. Lazy grades on every read — visiting the page triggers
// the grader to fill in any ungraded rows whose game_date is past.
...
export function WnbaCalibration() {
  useTitle(['WNBA Calibration']);
  const [report, setReport] = useState<WnbaCalibrationReport | null>(null);
...
function CalibrationView({ report }: { report: WnbaCalibrationReport }) {
    ...
    <div className="mlb-slate-result">
      {empty && (
        <div className="mlb-info-banner">
          ...
          stats. Publish a slate via <code>POST /api/wnba/slate/today</code>
          and check back after the games finish — the grader auto-runs
        ...
        <div className="mlb-projection">
          ...
          <div className="mlb-projection-grid">
            ...
            <Stat label="Hit rate (smoothed)" value={`${report.smoothedHitRate.toFixed(1)}%`}
              hint="Bayesian-smoothed against a 5-of-10 prior." />
...
function CalibrationVerdict({ report }: { report: WnbaCalibrationReport }) {
