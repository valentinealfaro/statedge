// WNBA Player drilldown modal — Phase 80. Mirrors NBA/MLB pattern.
// Click any player on /wnba/slate → modal with matchup, current game
// state, every line on the slate for that player, and live stat grid.
...
export type WnbaDrilldownPlayer = {
  athleteId: string;
  playerName: string;
  team: string | null;
};
...
export function WnbaPlayerDrilldown({
  player,
  combos,
  lines,
  onClose,
}: {
  player: WnbaDrilldownPlayer;
  combos: WnbaCombo[];
  lines: WnbaProjectedLine[];
  onClose: () => void;
}) {
  const [scoreboard, setScoreboard] = useState<WnbaScoreboardGame[] | null>(null);
  const [live, setLive] = useState<WnbaLiveTodayResponse | null>(null);

  useEffect(() => {
    let cancelled = false;
    Promise.all([
      getWnbaToday().then((d) => d.games).catch(() => []),
      getWnbaLiveToday().catch(() => null),
    ]).then(([games, liveFeed]) => {
      if (cancelled) return;
      setScoreboard(games);
      setLive(liveFeed);
  ...
  // Player lines: pull from combos first, then merge top-level lines
  // (top-level catalog isn't always restricted by combo gating).
  const playerLines: WnbaProjectedLine[] = [];
  const seen = new Set<string>();
  for (const combo of combos) {
    for (const l of combo.legs) {
      if (l.athleteId !== player.athleteId) continue;
  ...
  const livePlayer = live?.byPlayer[player.athleteId] ?? null;
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          <WnbaPlayerAvatar playerId={player.athleteId} name={player.playerName} size="lg" />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 20, fontWeight: 800 }}>{player.playerName}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
              {player.team && (
                <>
                  <WnbaTeamLogo abbr={player.team} name={player.team} size="md" />
        ...
        {game && (
          ...
          >
            ...
            <Link
              ...
              onClick={onClose}
              style={{ marginLeft: 'auto', color: '#b388ff', fontWeight: 700, fontSize: 12, textDecoration: 'none' }}
            >
              Full matchup →
            </Link>
          </div>
